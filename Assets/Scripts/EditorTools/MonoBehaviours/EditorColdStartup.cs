...
public class EditorColdStartup : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (!SceneManager.GetSceneByName(_persistentManagersSO.sceneReference.editorAsset.name).isLoaded)
        {
            ...
            //_pathStorage.lastPathTaken = null -- not something we need right now, since we only have one scene.
        }
        // CreateSaveFileIfNotPresent(); -- no save system in place yet
    }
    ...
    private void OnNotifyChannelLoaded(AsyncOperationHandle<LoadEventChannelSO> obj)
    {
        ...
        {
            // Raise a fake Scene Ready event, so that the player is spawned
